...
export class MenuPage extends Component {
    ...
    componentWillMount = () => {
        ...
        axios.get('http://18.220.10.109:3001/menu', {
            params: {
    ...
    deleteSectionButtonHandler = (e, sectionName) => {
        ...
        for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
            ...
            if (anItem.itemSection === sectionName) {
                axios.defaults.withCredentials = true;
		    axios.delete('http://18.220.10.109:3001/restaurantMenu', {
                    params: {
        ...
        axios.defaults.withCredentials = true;
	    axios.delete('http://18.220.10.109:3001/restaurantSection', {
            params: {
                restaurantId: restaurantId,
	...
	let sectionList = [];
	sectionList.push(...this.state.sections);
    	this.setState({
    ...
    saveMenuItemAddModal = (addItemState) => {
        ...
        axios.defaults.withCredentials = true;
	    axios.post('http://18.220.10.109:3001/restaurantMenu', data)
            .then(response => {
                if (response.status === 200) {
			...
			axios({
                        method: 'post',
                        url: 'http://18.220.10.109:3001/img/upload ',
                        data: formData,
