...
router.get('/buyerDetails', requireAuth,function (req, res) {
...
router.get('/restaurantDetails',requireAuth, function (req, res) {
...
router.post('/updateBuyerProfile', requireAuth,function (req, res) {
...
router.post('/img/upload', upload.single('selectedFile'), function (req, res) {
  ...
  console.log("file name in profile")
...
router.put('/remove/img', function (req, res) {
  ...
  console.log(req.body);
  var pathKafka = ""
  if (req.query.table == "restaurantMenuTable") {
    pathKafka = "removeItemPicture"
  }
  else if (req.query.table === "buyerTable") {
    pathKafka = "removeBuyerPicture"
  }
  else {
    pathKafka = "removeRestaurantPicture"
  }
  kafka.make_request('loginSignuptopic', { "path": pathKafka, "body": req.body }, function (err, result) {
    if (err) {
      console.log(err);
      res.status(500).json({ responseMessage: 'Database not responding' });
    }
