    ...
    if (getMenuResult) {
      ...
      for (index = 0; index < getMenuResult.length; index++) {
        ...
        //  console.log(anItem.items[index].item)
          ...
          console.log(anItem.items[j].item.itemImage);
          if (anItem.items[j].item.itemImage === null || anItem.items[j].item.itemImage === undefined || anItem.items[j].item.itemImage.length === 0) {
            // res.status(400).json({ responseMessage: 'Record not found' });
            console.log('No Image found for this item');
          } else if (typeof anItem.items[j].item.itemImage === "string") {
            console.log(anItem.items[j].item.itemImg);
            imageFilepath = path.join(__dirname, "../uploads/profilePictures", anItem.items[j].itemImage);
