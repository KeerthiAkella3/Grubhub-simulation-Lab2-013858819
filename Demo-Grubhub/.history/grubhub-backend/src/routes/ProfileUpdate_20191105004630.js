...
router.post('/updateBuyerProfile',  function (req, res) {
  ...
  kafka.make_request('loginSignuptopic',{"path":"updateBuyerProfile", "id": req.body.buyerId}, function(err,result){
    if (err) {
      console.log(err);
      res.status(500).json({ responseMessage: 'Database not responding' });
    }
...
router.get('/profile/img', function (req, res) {
  ...
  kafka.make_request('loginSignuptopic',{"path":"getPicture","body":req.query}, function(err,result){
    if (err) {
      console.log(err);
      res.status(500).json({ responseMessage: 'Database not responding' });
    }
...
router.put('/remove/img', function (req, res) {
  ...
  console.log("Request body:");
  console.log(req.body);
  kafka.make_request('loginSignuptopic',{"path":"removePicture", "body": req.body}, function(err,result){
    if (err) {
      console.log(err);
      res.status(500).json({ responseMessage: 'Database not responding' });
    }
