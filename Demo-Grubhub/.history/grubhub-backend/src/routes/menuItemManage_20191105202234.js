...
//var blag = require('./../../uploads/profilePictures')
    ...
    if (getMenuResult) {
      ...
      for (index = 0; index < getMenuResult.length; index++) {
        ...
        for (var j = 0; j < anItem.items.length; j++) {
          ...
          console.log(anItem.items[j].item);
          if (anItem.items[j].item.itemImg === null || anItem.items[j].item.itemImg === undefined || anItem.items[j].item.itemImg.length === 0) {
            // res.status(400).json({ responseMessage: 'Record not found' });

            console.log('No Image found for this item');
          } else if (typeof anItem.items[j].item.itemImg === "string") {
            console.log(anItem.items[j].item.itemImg);
          ...
          resItem = {
            ...
            itemName: anItem.items[j].item.itemName,
            itemDesc: anItem.items[j].item.itemDescription,
            itemPrice: anItem.items[j].item.itemPrice,
...
router.post('/restaurantMenu', function (req, res) {
  //
  console.log("in post method restaurantMenu backend");
  console.log("result")
  //console.log(res)
  ...
  console.log(req.body.menuItemImage)
  ...
  kafka.make_request('restaurantMenu', { "path": "addMenuItem", "restaurantId": restaurantId, "sectionName": req.body.menuItemSection, "itemData": menuItemAddData }, function (err, result) {
    ...
    console.log("resukt from kafka add menuItem")
