...
router.post('/img/upload', upload.single('selectedFile'), function (req, res) {
  ...
  kafka.make_request('loginSignuptopic',{"path":"uploadBuyerPicture", "body": req.body, "filename": filename}, function(err,result){
    if (err) {
      console.log(err);
      res.status(500).json({ responseMessage: 'Database not responding' });
    }
...
router.get('/profile/img', function (req, res) {
  ...
  if(req.query.table === "buyerTable"){
    path = "getBuyerPicture"
  }
  else{
    path = "getRestaurantPicture"
  }
  kafka.make_request('loginSignuptopic',{"path":path,"body":req.query}, function(err,result){
