      ...
      for (index = 0; index < getMenuResult.length; index++) {
        ...
        if (anItem) {    
          ...
          for (var j = 0; j < anItem.items.length; j++) {
            ...
            console.log("Image in backend----1")
            console.log(anItem.items[j].menuItemImage);
            
            if (anItem.items[j].menuItemImage === null || anItem.items[j].menuItemImage === undefined || anItem.items[j].menuItemImage.length === 0) {
              // res.status(400).json({ responseMessage: 'Record not found' });
              console.log('No Image found for this item');
            } else if (typeof anItem.items[j].menuItemImage === "string") {
              console.log("Image in backend")
              console.log(anItem.items[j].menuItemImage);
              imageFilepath = path.join(__dirname, "./../../uploads/profilePictures", anItem.items[j].menuItemImage);
