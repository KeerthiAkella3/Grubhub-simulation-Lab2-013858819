...
router.get('/buyerDetails', requireAuth, function (req, res) {
  ...
  kafka.make_request('profileTopics',{"path":"get_profile","body":req.query}, function(err,result){
    if (err) {
      console.log(err);
...
router.post('/profile', requireAuth, function (req, res) {
  ...
  kafka.make_request('profileTopics',{"path":"update_profile", "body": req.body}, function(err,result){
    if (err) {
      console.log(err);
      res.status(500).json({ responseMessage: 'Database not responding' });
    }
