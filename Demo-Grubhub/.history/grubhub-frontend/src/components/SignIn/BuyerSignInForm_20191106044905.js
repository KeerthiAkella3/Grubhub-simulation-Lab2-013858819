...
class BuyerSignInForm extends Component {
    ...
    submitBuyerLogin = (e) => {
        ...
        axios({
            method: 'post',
            url: 'http://localhost:3001/buyerLogin',
            data: {"jsonData" : JSON.stringify(data)},        
            data: data,
            config: { headers: { 'Content-Type': 'multipart/form-data' } }
            })
            .then(response => {
                ...
                if (response.data.validUser === true) {
                    ...
                    localStorage.setItem("cookie1", response.cookies.cookie1);
                    localStorage.setItem("cookie2", response.cookies.cookie2);
                    localStorage.setItem("cookie3", response.cookies.cookie3);
                    localStorage.setItem("cookie4", response.cookies.cookie4);
