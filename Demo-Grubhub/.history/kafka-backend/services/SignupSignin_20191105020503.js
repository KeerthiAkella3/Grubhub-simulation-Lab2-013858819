...
function updateBuyerProfile(msg, callback) {
    ...
    Users.findOneAndUpdate({ '_id': msg.id }, { $set:
        {
            "buyerName": msg.body.buyerName,
            "buyerEmail":msg.body.buyerEmailId,
            "buyerImg": msg.body.img,
            "buyerPhoneNumber": msg.body.buyerPhone,
            "buyerAddress": msg.body.buyerAddress
          }
...
function uploadBuyerPicture(msg, callback) {
    ...
    Users.findOneAndUpdate({ '_id': msg.body.id }, { $set: 
        {"buyerImg": msg.body.filename} 
    }, function (err, results) {
        if (err) {
            console.log(err);
            callback(err, "Database Error");
...
function getRestaurantPicture(msg, callback) {
    ...
    Restaurant.findOne({ '_id': msg.body.id}, {'_id': 0, 'img':1}, function (err, results) {
        if (err) {
            console.log(err);
            callback(err, "Database Error");
        } else {
...
function getBuyerPicture(msg, callback) {
    ...
    Users.findOne({ '_id': msg.body.id}, {'_id': 0, 'buyerImg':1}, function (err, results) {
        if (err) {
            console.log(err);
            callback(err, "Database Error");
        } else {
