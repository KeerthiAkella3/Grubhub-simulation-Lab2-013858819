...
function uploadItemPicture(msg, callback) {
    ...
    console.log("id")
    console.log(msg.body.id)
    Restaurant.findOneAndUpdate({ '_id': msg.body.id }, function (err, results) {
        if(results){
            console.log("Yaay restaurant");
            Restaurant.sections.forEach(function(section){
                if(section["sectionName"] == msg.body.sectionName){
                    Restaurant.sections.items.forEach(function(item){
                        if(item["itemName"]==msg.body.itemName){
                            items["itemImg"].push({msg.filename})
                        }
...
function getItemPicture(msg, callback) {
    ...
    Restaurant.findOne({ _id: msg.body.id }, function (err, results) {
     ...
     console.log(results)
     if(results){
         var img = msg.filename
         restaurant.sections.forEach(function (section) {
            console.log("before adding in img section");
            console.log(section);
