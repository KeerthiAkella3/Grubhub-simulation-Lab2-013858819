...
router.get('/buyerDetails', function (req, res) {
  ...
  kafka.make_request('loginSignuptopic',{"path":"buyerProfile","buyerId":req.query.buyerId}, function(err,kafkaResult){
    if (err) {
      console.log(err);
...
router.post('/updateBuyerProfile',  function (req, res) {
  ...
  kafka.make_request('loginSignuptopic',{"path":"updateBuyerProfile", "body": req.body}, function(err,result){
    if (err) {
      console.log(err);
      res.status(500).json({ responseMessage: 'Database not responding' });
    }
...
router.post('/img/upload', upload.single('selectedFile'), function (req, res) {
  ...
  kafka.make_request('loginSignuptopic',{"path":"upload_profile_pic", "body": req.body, "filename": filename}, function(err,result){
    if (err) {
      console.log(err);
      res.status(500).json({ responseMessage: 'Database not responding' });
    }
...
router.get('/profile/img', function (req, res) {
  ...
  kafka.make_request('loginSignuptopic',{"path":"get_profile_img","body":req.query}, function(err,result){
    if (err) {
      console.log(err);
      res.status(500).json({ responseMessage: 'Database not responding' });
    }
...
router.put('/remove/img', function (req, res) {
  ...
  console.log("Request body:");
  console.log(req.body);
  kafka.make_request('loginSignuptopic',{"path":"remove_profile_img", "body": req.body}, function(err,result){
    if (err) {
      console.log(err);
      res.status(500).json({ responseMessage: 'Database not responding' });
    }
