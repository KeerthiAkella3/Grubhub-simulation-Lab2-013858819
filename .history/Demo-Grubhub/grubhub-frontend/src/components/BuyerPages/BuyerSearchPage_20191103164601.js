...
export class BuyerSearchPage extends Component {
    ...
    render() {
        ...
        // Pagination
        let active = thi.state.paginationActive;     // Initially active key
        let items = [];     // 
        for (let number = 1; number <= Math.ceil(searchResults.length / 5); number++) {
            let isActive = (number === active);
            items.push(
                <Pagination.Item key={number} active={isActive} onClick={this.onClickPageItem}>
                    {number}
                </Pagination.Item>,
            );
        }
