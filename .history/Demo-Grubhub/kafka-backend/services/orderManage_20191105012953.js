...
function getRestaurantOrders(msg, callback) {
  ...
  let resturantOrderStatus = msg.restaurantOrderStatus;
  Orders.find({
    "restaurantId" : restaurantId, 
  }, function (err, allOrders) {
    let orderData = [];
    let index = 0;
    for (index = 0; index < allOrderData.length; index++) {
      if (allOrderData[index].restaurantOrderStatus === restaurantOrderStatus) {
        orderData.push(allOrderData[index]);
      }
    }
    if (orderData && !err) {
      console.log("Received information from mongodb on buyer order details");
