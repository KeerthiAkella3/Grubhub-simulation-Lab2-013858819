...
function sendMessagesToBuyer(msg, callback) {
  console.log("In add place order service. Msg: ", msg);
  let restaurantId = msg.fromId;
  let buyerId = msg.toId;
  let isCreate = false;
  let messages = [];
  Orders.findOne({ restaurantId: fromId, buyerId: toId }, function (err, result) {
    if (err) {
      isCreate = true;
    } else {
...
function sendMessagesToOwner(msg, callback) {
  ...
  let isCreate = false;
  let messages = [];
  Orders.findOne({ fromId: fromId, toId: toId }, function (err, result) {
    if (err) {
      isCreate = true;
    } else {
      console.log("Found messages between " + fromId + " and " + toId);
